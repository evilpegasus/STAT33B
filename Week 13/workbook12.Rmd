...
The `str_sub` function subsets strings:
```{r}
library(stringr)
x = "Hello world"
str_sub(x, 1, 5)  # Get the first 5 characters ("Hello")
str_sub(x, -5) # Get the last 5 characters ("world")
str_sub(x, -5) = "everybody"  # Replace the last 5 characters with "everybody"
x
```
...
library(tidyr)
table3
splits = str_split_fixed(table3$rate, "/", 2)
tidy_table3 = table3[-3]
tidy_table3["cases"] = splits[,1]
tidy_table3["population"] = splits[,2]
tidy_table3
```
...
has_backslash(c(r"(no backslash here)", r"(here \is \a \backslash)"))
```
