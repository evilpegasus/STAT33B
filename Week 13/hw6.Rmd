...
After preallocating `chain`, the function took `850ms` to run, almost 50 times faster.
```{r, eval=FALSE}
...
After changing the samples to integers, the function took `720ms` to run. This is slightly faster than the previous version that used strings.
```{r, eval=FALSE}
# Your code goes here.
profvis({
...
The function now takes just `240ms` to run, much faster than before.
```{r, eval=FALSE}
# Your code goes here.
profvis({
  sample_markov = function(n, p = c(0.6, 0.7), init = 0) {
  ...
  for (i in 1:n) {
    ...
    if (chain[i] == 0) {
      # Randomly sample new state using the probabilities.
      chain[i + 1] = rbinom(1, 1, p[1])

    } else {
      # Randomly sample new state using the probabilities.
      chain[i + 1] = rbinom(1, 1, p[2])
    }
  }
...
The function now takes just `50ms` to run. Compared to the starter code, this is about 1000 times faster.
```{r, eval=FALSE}
# Your code goes here.
profvis({
  sample_markov = function(n, p = c(0.6, 0.7), init = 0) {
  ...
  for (i in 1:n) {
    ...
    if (chain[i] == 0) {
      # Randomly sample new state using the probabilities.
      chain[i + 1] = as.integer(sample[i] > p[1])

    } else {
      # Randomly sample new state using the probabilities.
      chain[i + 1] = as.integer(sample[i] > p[2])
    }
  }
