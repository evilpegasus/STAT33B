...
locate = function(name, env) {
   e = env
   while(!exists(name, e, inherits = FALSE)) {
...
```
### Part 4
The `methods` package provides `show()`
```{r}
locate("show", e)
```
...
`find_fib3` will use more memory than `find_fib2` but will be much faster. Usually speed is preferred, so `find_fib3` would be better.

### Part 2

`find_fib3` is much faster (about 20x faster in these two tests).
```{r}
library(microbenchmark)
n = 30
microbenchmark(find_fib2(n))
microbenchmark(find_fib3(n))
