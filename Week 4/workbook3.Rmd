...
The `==` operator is a short-circuited **AND** operator. If the left side of the operator is false, then the entire experssion will be false. There is no need to check the right side of an **AND** operator when the left is already false.
...
1. The first element is the list with `(1i, 2, 3i)` and the second is a list with `("hello", "hi")`.
```{r}
mylist[[1]]
mylist[[2]]
```
...
mylist[[c(1, 2)]]
```
Passing a vector into the index of `[[` for recursive lists gets elements of the nested list.
